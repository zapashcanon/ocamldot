...
let fatal_error =
  ref (fun s ->
      prerr_endline s ;
      prerr_endline
        (Printf.sprintf "You way have a look at %s for details." !log_file) ;
      exit 1)
...
let is_suffix ~suf s =
  ...
  len_s >= len_suf && String.sub s (len_s - len_suf) len_suf = suf
...
let create_temp_file ?(ext = !temp_files_ext) ?(contents = "") () =
  let file, oc = Filename.open_temp_file !temp_files_prefix ("." ^ ext) in
  output_string oc contents ; close_out oc ; file

let remove_empty_strings = List.filter (( <> ) "")

let string_of_includes l =
  String.concat " "
    (List.map (fun s -> "-I " ^ Filename.quote s) (remove_empty_strings l))
...
let access_ok_errors = [EACCES; EROFS; ENOENT; ENOTDIR; ELOOP]
...
let testfile flags filename =
  ...
  try
    ...
    let rec test = function
      ...
      | Fnonempty ->
          st.st_size > 0
      | Fnewer string ->
          st.st_mtime > (stat string).st_mtime
      | Folder string ->
          st.st_mtime < (stat string).st_mtime
      | Fequal string ->
  ...
  with Unix.Unix_error (err, _, _) when List.mem err access_ok_errors -> false
...
let string_from_descr fd =
  let rec readfd accu =
    ...
    | 0 ->
        Bytes.concat (Bytes.of_string "") accu
    | n ->
        let str_buf =
          if n < buffer_size then Bytes.sub str_buf 0 n else str_buf
        in
        readfd (str_buf :: accu)
  in
...
let unlink_f file =
  if !debug then ()
    ...
    in
    let f file = try unlink file with Unix_error (ENOENT, _, _) -> () in
    List.iter f files
...
let execvp_redirect redirections cmd args =
  ...
  let temp_file =
    if
      List.exists
        (function In_from_string _ -> true | _ -> false)
        redirections
  ...
  let rec make_redirect rd =
    ...
    | Silent ->
        make_redirect Out_null ; make_redirect Err_to_out
  in
  match fork () with
  | 0 ->
      let exec () =
  ...
  | pid ->
      ...
      (match temp_file with Some tmp -> unlink_f tmp | _ -> ()) ;
      res

let execvp cmd arg =
  match fork () with
  | 0 ->
      Unix.execvp cmd (Array.concat [[|cmd|]; arg])
  | p ->
      snd (waitpid [] p)
...
let exec_and_get_first_line com args =
  match handle_unix_error (fun () -> execvp_to_list com args) () with
  | [] ->
      ""
  | h :: _ ->
      h
...
let exec_status_ok = function Unix.WEXITED 0 -> true | _ -> false

(** {2 Writing to log file} *)

let string_of_date t =
  let d = Unix.localtime t in
  Printf.sprintf "%02d/%02d/%02d %02d:%02d:%02d" (d.Unix.tm_year + 1900)
    (d.Unix.tm_mon + 1) d.Unix.tm_mday d.Unix.tm_hour d.Unix.tm_min
    d.Unix.tm_sec
...
let version_of_string s =
  ...
  try List.map int_of_string l with Failure _ -> dummy_version

(** [string_of_version v] returns a string to display the given version.
   For example, [string_of_version [1;2;3] = "1.2.3"]. *)
let string_of_version v = String.concat "." (List.map string_of_int v)
...
let ocaml_libdir conf =
  match
    handle_unix_error (fun () -> execvp_to_list conf.ocamlc [|"-v"|]) ()
  with
  | [] | [_] ->
      ""
  | _ :: l :: _ -> (
...
let check_version ?(on_err = !fatal_error) version prog =
  ...
  | _ -> (
    match
      handle_unix_error (fun () -> execvp_to_list prog [|"-version"|]) ()
    with
    | [] ->
...
let check_conf_versions conf =
  List.iter
    (check_version conf.version_string)
    [ conf.ocamlopt
    ; conf.ocamldep
    ; conf.ocamldoc
    ; conf.ocamllex
...
let ocaml_conf ?(withopt = false) ?(ocamlfind = false) () =
  ...
  !print
    (Printf.sprintf "found OCaml version %s (%s)\n"
...
let string_of_mode = function `Byte -> "byte" | `Opt -> "opt"
...
let string_of_com_args com args =
  Printf.sprintf "%s %s" com
    (String.concat " " (List.map Filename.quote (Array.to_list args)))
...
let can_compile mode conf ?(includes = []) ?(options = []) file =
  ...
  let args =
    Array.of_list
      ( "-c"
        :: List.flatten
             (List.map (fun s -> ["-I"; s]) (remove_empty_strings includes))
      @ options @ [file] )
  in
  add_to_log (string_of_com_args ocamlc args) ;
  exec_status_ok
    (execvp_redirect [Out_append_to_file !log_file; Err_to_out] ocamlc args)
  ...
  let res =
    handle_unix_error
      (fun () ->
        ...
        try_finalize
          (can_compile mode conf ?includes ?options)
          file unlink_f file)
      ()
  in
...
let can_link ?mes mode conf ?out ?(includes = []) ?(libs = []) ?(options = [])
    files =
  (match mes with None -> () | Some s -> !print s) ;
  let ocamlc = ocamlc_of_mode conf mode in
  let libs =
    match mode with `Byte -> libs | `Opt -> List.map byte_ext_to_opt libs
  in
  let files =
    match mode with `Byte -> files | `Opt -> List.map byte_ext_to_opt files
  in
  let outfile =
    match out with None -> create_temp_file ~ext:"out" () | Some f -> f
  in
  let args =
    Array.of_list
      ( "-linkall" :: "-o" :: outfile
...
let ocamlfind_query_version conf package =
  ...
  | _ -> (
    try
      match
        exec_and_get_first_line conf.ocamlfind
...
let check_ocamlfind_package ?min_version ?max_version ?(fail = true) ?not_found
    conf name =
  ...
  let not_found =
    ...
    | None -> (
        ...
        | `Package_not_installed pkg ->
            ...
            if fail then !fatal_error msg else !print msg
        | `Package_bad_version version ->
            let msg =
              Printf.sprintf
                "Package %s found with version %s, but wanted %s%s%s" name
                version
                ( match min_version with
                | None ->
                    ""
                | Some v ->
                    Printf.sprintf ">= %s" (string_of_version v) )
  ...
  | None ->
      not_found (`Package_not_installed name) ;
      false
  | Some s -> (
    match ocamlfind_query_version conf name with
    | None ->
        not_found (`Package_bad_version "<no version>") ;
        false
    | Some s ->
...
let subst_val var = try Hashtbl.find substs var with Not_found -> ""
...
let output_substs_to_file file =
  let contents =
    String.concat "\n"
      (List.map
         (fun (var, v) -> Printf.sprintf "%s=\"%s\"" var v)
         (get_substs_list ()))
  in
...
let _ =
  if conf.version << ocaml_required then (
    let msg =
      Printf.sprintf "Error: OCaml %s required, but found %s.\n"
        (string_of_version ocaml_required)
        (string_of_version conf.version)
    in
    !print msg ; exit 1 )
...
let () =
  if
    check_ocamlfind_package conf ~fail:false "lablgtk2"
...
let _ =
  ...
  else output_substs_to_file Sys.argv.(1)
